...
description: >
  ...
  adding, auditing, superseding, or reviewing decisions in Decision Logs and
  standalone Decision Records. Checks every new decision for superseded active
  records, classifies records by future decision value, and maintains a focused
  active corpus without erasing history that can still guide work.
type: prompt
whenToUse: When adding, auditing, superseding, or reviewing decisions in Decision Logs or standalone Decision Records
---
...
Decisions live in two containers, distinguished by scope, not by importance:

| Container | Scope | Lifecycle |
|---|---|---|
| Decision Log (`.decisions.md`) | one feature; created with its spec | entries are appended once and never deleted; no state machine |
| Standalone Decision Record (`docs/specs/decisions/`) | a durable cross-feature or cross-spec topic | proposed → accepted / rejected; superseded as a marker |

A feature-bound decision goes in that feature's Decision Log and stays there. A
decision that constrains or outlives a single feature becomes a standalone
Decision Record. A decision has exactly one home; other files reference it, they
do not copy its rationale.

## Decision Record locations

In the spec-loop workflow, decision records live in these places:

| Location | Format | Written by | Lifecycle |
|---|---|---|---|
| Decision Log (`.decisions.md`) | D1, D2, ... continuous per feature | write-spec (brainstorm entries) + main agent | Appended once, never deleted |
| Standalone DR files (`docs/specs/decisions/`) | Full DR format, one of three states | main agent | proposed → accepted / rejected; superseded |

The Decision Log is the primary working document. Standalone DR files hold
durable cross-feature or cross-spec proposals and decisions: architecture
choices, simplification-audit proposals, and research guardrails. Specs frozen
before this model may still carry a `## Decisions` section; treat it as a frozen
historical snapshot, never as a place to add records.

## Check supersession when adding a decision

Every new decision triggers a scoped check of existing records covering the same topic. Before writing:
...
Apply these outcomes when reviewing existing Decision Records. Audits are
...
- **Proposed (standalone) — keep until resolved:** a proposed Decision Record is never deleted; keep it until it is implemented (migrated to accepted) or evaluated and rejected.
...
1. In the old record (a Decision Log entry or a standalone file), append:
   `**Status:** superseded by [D<n> in <location>]` or `by [Standalone DR: <title>]`
...
4. For full supersession, transfer every unique rationale, alternative, consequence, and named coverage gap into the current owner before marking the old record
5. If the old record sits in a legacy frozen spec's `## Decisions` section (specs frozen before this model), do not edit the spec — append the supersession note to that feature's Decision Log instead

## Standalone Decision Record lifecycle

Standalone files are the only decision artifact with a state machine. The state
is the value of the `Status:` line; each state binds its own section skeleton.

| State | Meaning | Sections |
|---|---|---|
| `Status: proposed` | durable proposal, not yet implemented | `## Problem` / `## Proposal` / `## Alternatives Considered` / `## Acceptance criteria` / `## Risks` |
| `Status: accepted` | implemented; describes shipped reality | `## Problem` / `## Decision` / `## Alternatives Considered` / `## Consequences` |
| `Status: rejected` | evaluated and declined | the sections it was written with, frozen |
...
- **proposed → accepted:** when the decision first ships, the main agent
  ...
  change merges. Partial implementations note remaining gaps in `## Consequences`.
- **proposed → rejected:** add `rejected — <reason, one line>` to the `Status:`
  line and freeze the sections. The file stays as a guardrail.
...
Date: YYYY-MM-DD
Specs: [<spec-1>](../<slug>.md), [<spec-2>](../<slug>.md)   <!-- for accepted records -->
...
- **reviewer / main agent:** reference it as decisions evolve during implementation and review; the main agent executes state transitions.
