...
`brainstorm (requirements) → write-spec (review → freeze spec) → dispatch loop (implement/commit/review/fix) → acceptance against spec`
...
- **The spec passes a review gate before freezing**: the `reviewer` reviews read-only → revise → re-review until `approve`; in interactive mode the user confirms the reviewed draft before freezing; unattended freezes automatically once approved, with no user step (details in `write-spec`).
...
- **Unattended (goal)**: the spec passes the review gate before freezing; contract slices go review → follow-up fixes → incremental re-review; mechanical slices commit on test evidence (L0/L1) and get a batch review when their wave completes; after the whole spec passes, **merge the PR + delete the branch** (may use `gh pr merge`).
- **Budget check**: at slice/batch boundaries, compare remaining budget against remaining slices; if insufficient → stop and report (the Progress board guarantees recoverability); never degrade quality and keep running.
- **Unattended fixes blockers only**: suggestions are recorded and reported, not chased.
...
- **Dispatch guidance (no template)**: different harnesses have different dispatch standards; the specific prompt is decided by the main agent per task type at dispatch time. Suggested emphasis — implementation: `[Sn]` verbatim text + related standalone DRs + boundaries and prohibitions; investigation: hypotheses + evidence standards; review: spec path + diff scope; spec reviews and outline skeleton reviews dispatch `zhipuai-coding-plan/glm-5.3` (high), attaching the spec draft + requirements baseline + the Decision Log brainstorm entries or the outline + the skeleton checklist.
...
- **Reviewer rejection limit**: the same review target (slice / batch / whole branch / spec draft / outline skeleton) rejected with `needs fixes`/`reject` 3 times in a row → stop and report (see stop list); a green light resets the count; deviation findings do not count.

## GitHub Flow conventions (non-skill)

- 1 spec = 1 branch = 1 PR: the spec branch is `spec/<slug>`, slice branches are `spec/<slug>-s<n>`; independent slices cut from main, dependent slices cut where their dependency is met — slice-level stacking (PRs follow the same rule), merge-only updates, rebase/force-push banned; `main` is always deployable.
- **Merge-in rules**: the main agent merges serially in dependency order with merge commits; a contract slice merges into the spec branch only after its review returns `approve`; a mechanical slice merges on L0/L1 evidence and gets a batch review when its wave completes.
- **Commit format**: one `[Sn]` one main commit (`[S2] feat: <summary>`) + follow-up commits (`[S2] review: <what was fixed>`); a git pre-commit hook runs L0 (details in `references/workflow/testing.md`).
- **Pre-merge spec gate**: the diff does not touch `docs/specs/<slug>.md`; merge prerequisites: reviewer `approve` + evidence exists.
- **Governance direct-push list**: the spec + Decision Log freeze commit, Progress updates, and standalone DR status transitions (proposed→accepted/rejected) are committed directly on main by the main agent (no branch/PR); after freezing, decisions/Progress are pushed directly to main only and never written on spec branches; each transition is re-checked at the next review or audit.
- `--no-verify` sits on the same ban list as force-push; branch/commit/PR details are in `references/workflow/github-flow.md`.
...
Boundary notes — attribution of four easily confused cases:
...
- **New test red** (a newly written test fails) **is not** a `real regression red`: it belongs to the fix loop — tracked by an independent counter; 3 consecutive red rounds in the same slice → stop and report; a green light resets the count; only green-turning-red is a `real regression red` and hits the existing stop item.

The numeric threshold for `Reviewer consecutive rejections at limit` is in the “Dispatch-loop run parameters” section.

## Review gate

- Contract slices: after each completed `[Sn]`, dispatch the `reviewer` (review → follow-up fixes → incremental re-review); mechanical slices are exempt from per-slice review and are covered by the batch review at wave completion or the whole-spec review; check spec compliance (Claims, incl. `Global Constraints`/`Out of Scope` not violated) + the two-layer structured review.
